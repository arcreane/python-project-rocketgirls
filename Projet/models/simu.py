...
class Simulation:
    def __init__(self):
        ...
        self.temps_debut = 0
        self.temps_dernier_niveau = 0  # ← AJOUTÉ : pour tracker le temps du dernier changement de niveau

    def demarrer_simulation(self):
        """Démarre la simulation"""
        self.simulation_en_cours = True
        self.temps_debut = self.temps_ecoule
        self.temps_dernier_niveau = self.temps_ecoule  # ← AJOUTÉ
    ...
    def ajouter_avion_aleatoire(self):
        ...
        # Cap aléatoire au lieu de toujours vers le centre
    ...
    def mettre_a_jour(self, delta_temps: float):
        ...
        for avion in self.avions:
            ...
            # Perte de points pour les urgences non résolues (-1 point par seconde)
            if avion.urgence != TypeUrgence.AUCUNE:
                avion.temps_urgence += delta_temps  # Compteur de temps d'urgence

                # Perdre 1 point par seconde d'urgence
                ...
                if perte_points > avion.points_urgence_perdus:
                    ...
                    if points_a_perdre > 0:
                        print(f"⚠️ {avion.identifiant} - Urgence non résolue: -{points_a_perdre} point(s)")

            # Vérification de l'atterrissage final (avion au sol sur la zone verte)
    ...
    def demander_atterrissage(self):
        ...
        # Vérifier les conditions pour passer en mode APPROCHE
        if distance < 1000 and avion.position.altitude < 600:
            avion.preparer_atterrissage()
            print(f"✈️ {avion.identifiant} autorisé pour l'approche finale!")
